...
def load_config():
    ...
    if config_path.exists():
        try:
            ...
            # 确保列表字段存在
            if "extra_voice_pool" not in CONFIG:
                CONFIG["extra_voice_pool"] = []
            if "admin_whitelist" not in CONFIG:
...
def check_permission(event: AstrMessageEvent) -> bool:
    ...
    try:
        if hasattr(event, 'context') and hasattr(event.context, 'get_admin_ids'):
            admin_ids = event.context.get_admin_ids() or []
        elif hasattr(event, 'get_admin_ids'):
            admin_ids = event.get_admin_ids() or []
    except Exception:
        pass
...
async def handle_message(event: AstrMessageEvent):
    ...
    try:
        ...
        text = message_chain.text().strip() if message_chain.text() else ""
        ...
        if text.startswith("/"):
            ...
            if cmd == "/voice.delete":
                ...
                if target_file:
                    try:
                        target_file.unlink()
                        pool = CONFIG.get("extra_voice_pool", [])
                        if isinstance(pool, list) and name in pool:
                            pool.remove(name)
                            CONFIG["extra_voice_pool"] = pool
                            save_config()
            ...
            if cmd == "/voice.list":
                ...
                if not current_keys:
                    ...
                    for k in current_keys:
                        is_user = k in pool if isinstance(pool, list) else False
                        tag = "🟢" if is_user else "🔵"
                        msg += f"{tag} `{k}`\n"
            ...
            if cmd == "/voice.check":
                ...
                await event.send(f"当前用户权限状态：{status}\n模式：{CONFIG.get('admin_mode', 'whitelist')}")
                return
        ...
        if keyword in VOICE_MAP:
            ...
            if os.path.exists(file_path):
                ...
                pool = CONFIG.get("extra_voice_pool", [])
                if isinstance(pool, list) and keyword in pool:
                    pool.remove(keyword)
                    CONFIG["extra_voice_pool"] = pool
                    save_config()
    except Exception as e:
        logger.error(f"[{__plugin_name__}] 消息处理异常：{e}")
...
def init_plugin(context):
    ...
    try:
        ...
        logger.error(f"[{__plugin_name__}] 创建目录失败：{e}")
        EXTRA_VOICE_DIR = PLUGIN_PATH / "extra_voices"
        EXTRA_VOICE_DIR.mkdir(parents=True, exist_ok=True)
